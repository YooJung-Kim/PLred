...
def script_match_timestamps(configname):
    '''
    Match timestamps between two cameras: fast camera TO slow camera.
    
    Config file should specify:
    - which camera is PSF camera (for cropping/processing)
    ...
    Config file example:
        ...
        [PSFCamera]
        camera_type         = fast  # or 'slow' - which camera is the PSF camera
        ...
        verbose             = False
        show_plot           = True
        crop_width          = 20  # only applied to PSF camera
    '''
    
    config = ConfigObj(configname)
    
    # Identify which camera is which
    psfcam_is_fast = config['PSFCamera']['camera_type'].lower() == 'fast'
    
    # Read both cameras' configurations
    ...
    # Match fast camera TO slow camera (reference)
    ...
    for ind in tqdm(np.arange(ind_start, ind_end-1)):
        leftfrac = (fastcam_timestamp[bisect_inds[ind]] - slowcam_timestamp[ind]) / \
                   (fastcam_timestamp[bisect_inds[ind]] - fastcam_timestamp[bisect_inds[ind]-1])
        rightfrac = (slowcam_timestamp[ind+1] - fastcam_timestamp[bisect_inds[ind+1]-1]) / \
                    (fastcam_timestamp[bisect_inds[ind+1]] - fastcam_timestamp[bisect_inds[ind+1]-1])
    ...
    ####################################
    # Process and average fast camera  #
    # (only crop if it's PSF camera)   #
    ####################################
    ...
    # Apply cropping ONLY if fast camera is the PSF camera
    ...
    if psfcam_is_fast and crop_width is not None:
        ...
        print(f"Fast camera is PSF camera - applying crop: [{xc-xw}:{xc+xw}, {yc-yw}:{yc+yw}]")
        fastcam_avgdark = fastcam_avgdark[xc-xw:xc+xw, yc-yw:yc+yw]
    else:
        xw, yw = xwidth//2, ywidth//2
        print("Fast camera is not PSF camera or no crop specified - saving full frames")
    
    # Stack matched frames
    ...
    nstacks = []
    # stacked_frames = []
    stacked_frames = open_memmap(
        '%s_fastcam_matched_frames.npy' % (outname+'/'+filename),
        mode='w+',
        dtype='float32',
        shape=(len(indices), 2*xw, 2*yw)
    )
    ...
    for ind in tqdm(indices):
        ...
        for (_fastcam_ind, _fastcam_frac) in zip(fastcam_ind, fastcam_frac):
            ...
            if verbose:
                print("appending frame", fastcam_frameinds[_fastcam_ind],'with fraction of', _fastcam_frac)
        ...
        stacked_frames[ind] = np.sum(stacked_frame, axis=0)
    ...
    data_to_save = {
        ...
        'psfcam_is_fast': psfcam_is_fast  # Store this info
    }
...
#     Key terminology:
#     - PSF camera: the camera imaging the PSF (for centroid measurements)
#     - PL camera: the photonic lantern camera (contains spectral data)
#     - Fast/Slow: determined by frame rate, not by camera type
    
#     Either PSF or PL camera can be the fast/slow camera depending on observation setup.
#     '''
...
#                  plcam_spec_path,
#                  fastcam_frames_name, 
#                  fastcam_timestamp_name,
#                  obs_start, 
#                  obs_end,
#                  psfcam_is_fastcam=True,  # NEW: explicitly specify which camera is which
#                  match_frames=True,
#                  pix2mas=16.2,
...
#         '''
#         Initialize the FrameSorter
        
#         Parameters
#         ----------
#         plcam_timestamp_path : str
#             Path to PL camera timestamp files
#         plcam_spec_path : str
#             Path to PL camera spectral data files
#         fastcam_frames_name : str
#             Path to matched fast camera frames (output from script_match_timestamps)
#         fastcam_timestamp_name : str
#             Path to timestamp matching pkl file
#         obs_start : str (HH:MM:SS)
#             Start time of observation
#         obs_end : str (HH:MM:SS)
#             End time of observation
#         psfcam_is_fastcam : bool
#             True if PSF camera is the fast camera, False if PL camera is the fast camera
#         match_frames : bool
#             Whether to match frames between cameras
#         pix2mas : float
#             Plate scale in mas/pixel for PSF camera
#         plcam_header : str
#             File header for PL camera files
#         plcam_footer : str
#             File footer for PL camera files (e.g., '.fits' or '_spec.fits')
#         plcam_shape : tuple
#             Shape of PL camera frames (ny, nx)
#         '''
...
#         self.fastcam_frames_name = fastcam_frames_name
#         self.fastcam_timestamp_name = fastcam_timestamp_name
...
#         # Try to read from timestamp file if available
#         try:
#             with open(fastcam_timestamp_name, 'rb') as f:
#                 timestamp_data = pickle.load(f)
#                 if 'psfcam_is_fast' in timestamp_data:
#                     self.psfcam_is_fastcam = timestamp_data['psfcam_is_fast']
#                     print(f"Read camera configuration from timestamp file: PSF camera is {'fast' if self.psfcam_is_fastcam else 'slow'}")
#         except:
#             print(f"Using provided camera configuration: PSF camera is {'fast' if self.psfcam_is_fastcam else 'slow'}")
...
#         '''
#         Match frames from the two cameras and correctly assign to PSF/PL cameras
        
#         Logic:
#         - PL camera data is always from plcam_spec_path (spectral data)
#         - Fast camera data is from the matched frames file
#         - Assignment depends on psfcam_is_fastcam flag
#         '''
        
#         # Find PL camera files (always has spectral data)
#         plcam_timestampfiles = find_data_between(
#             self.plcam_timestamp_path, self.obs_start, self.obs_end, 
#             header=header, footer='.txt'
#         )
#         plcam_specfiles = find_data_between(
#             self.plcam_spec_path, self.obs_start, self.obs_end, 
#             header=header, footer=footer
#         )
        
#         # Load PL camera timestamps
#         timestamps_plcam = np.concatenate([np.genfromtxt(file)[:,4] for file in plcam_timestampfiles])
        
#         # Load fast camera matched frames and timestamps
#         fastcam_frames = np.load(self.fastcam_frames_name)
#         with open(self.fastcam_timestamp_name, 'rb') as f:
#             fastcam_timestamp = pickle.load(f)
        
#         # Get file and frame indices for PL camera
#         plcam_file_indices = []
#         plcam_frame_indices = []
#         for fi, f in enumerate(plcam_specfiles):
#             nframes = fits.getheader(f)['NAXIS3']
#             for n in range(nframes):
#                 plcam_file_indices.append(fi)
#                 plcam_frame_indices.append(n)
#         plcam_file_indices = np.array(plcam_file_indices)
#         plcam_frame_indices = np.array(plcam_frame_indices)
        
#         # Validate timestamps between PL camera and fast camera
#         # Note: if PSF is fast, we match fastcam TO plcam
#         #       if PL is fast, we match plcam TO fastcam
#         if self.psfcam_is_fastcam:
#             # PSF camera is fast, PL camera is slow (reference)
#             # Fast camera was already matched to PL camera in script_match_timestamps
#             idx_plcam, idx_fastcam = validate_timestamp_matching(
#                 timestamps_plcam, 
#                 np.array(fastcam_timestamp['timestamps'])
#             )
#         else:
#             # PL camera is fast, PSF camera is slow (reference)
#             # Need to match PL to fast camera
#             idx_fastcam, idx_plcam = validate_timestamp_matching(
#                 np.array(fastcam_timestamp['timestamps']),
#                 timestamps_plcam
#             )
        
#         # Filter based on validated timestamps
#         plcam_file_indices = plcam_file_indices[idx_plcam]
#         plcam_frame_indices = plcam_frame_indices[idx_plcam]
#         fastcam_frames = fastcam_frames[idx_fastcam]
#         timestamps = np.array(fastcam_timestamp['timestamps'])[idx_fastcam]
        
#         # Assign to PSF and PL cameras based on configuration
#         if self.psfcam_is_fastcam:
#             # PSF camera is the fast camera
#             self.psfcam_frames = fastcam_frames
#             self.plcam_file_indices = plcam_file_indices
#             self.plcam_frame_indices = plcam_frame_indices
#             self.plcam_files = plcam_specfiles
#         else:
#             # PL camera is the fast camera
#             # Fast camera matched frames are actually PL camera frames
#             self.plcam_frames_preloaded = fastcam_frames  # These are actually PL frames
#             self.psfcam_file_indices = plcam_file_indices  # These refer to PSF camera
#             self.psfcam_frame_indices = plcam_frame_indices
#             self.psfcam_files = plcam_specfiles  # In this case, these are PSF files
        
#         self.timestamps = timestamps

#     def compute_psfcam_centroids(self, peak=True):
#         '''
#         Compute centroids on PSF camera frames.
#         Always processes PSF camera, regardless of which is fast/slow.
#         '''
#         centroids = []
        
#         if self.psfcam_is_fastcam:
#             # PSF frames are already loaded (they were the fast camera)
#             psfcam_frames_to_process = self.psfcam_frames
#         else:
#             # PSF camera was slow, need to load from files
#             print("Loading PSF camera frames (PSF camera was the slow camera)...")
#             psfcam_frames_to_process = []
#             current_file = None
#             current_fileind = -1
            
#             for fileind, frameind in tqdm(zip(self.psfcam_file_indices, self.psfcam_frame_indices)):
#                 if fileind != current_fileind:
#                     current_fileind = fileind
#                     current_file = fits.getdata(self.psfcam_files[fileind])
#                 psfcam_frames_to_process.append(current_file[frameind])
            
#             psfcam_frames_to_process = np.array(psfcam_frames_to_process)
        
#         # Compute centroids on PSF frames
#         for t in range(len(psfcam_frames_to_process)):
#             try:
#                 if peak:
#                     cent = subpixel_centroid_2d(psfcam_frames_to_process[t])
#                 else:
#                     cent = center_of_mass(psfcam_frames_to_process[t])
#             except:
#                 cent = (np.nan, np.nan)
...
#         # Store PSF frames for later use
#         if not self.psfcam_is_fastcam:
#             self.psfcam_frames = psfcam_frames_to_process
...
def bin_by_centroids_to_file(outname, psfcamframes, plcam_file_indices, plcam_frame_indices, plcam_files, ny, nx,
            ...
            if num_frames[i,j] > 0:
                ...
                with h5py.File(outname+'_bin_%d_%d.h5' % (i,j), 'w') as h5f:
                    ...
                    if timestamps is not None:
                        timestamps_dset = h5f.create_dataset('timestamps',
                                                            data = timestamps[idx])
    ...
    if not skip_frame_reading:
        ...
        for fileind, f in tqdm(enumerate(plcam_files)):
            ...
            for i0, i in enumerate(id_frame): #range(len(id_frame)):
                ...
                try:
                    ...
                    print("KeyboardInterrupt caught, stopping frame reading")
                    raise
        ...
        return psfcam_binned_frames, num_frames, idxs
        # except KeyboardInterrupt:
        #     print("KeyboardInterrupt caught, returning partial results")
        #     return psfcam_binned_frames, num_frames, idxs
...
def bin_by_centroids_to_file2(outname, psfcamframes, plcam_frames_mmap, ny, nx,
                                  centroids, xbins, ybins,
                                  skip_frame_reading = False,
                                  nbin = 1,
                                  timestamps = None):
    ...
    num_frames = np.zeros((len(xbins)-1, len(ybins)-1))
    idxs = np.zeros((len(xbins)-1, len(ybins)-1, len(plcam_frames_mmap)), dtype=bool)
...
class FrameSorter:
    ...
    def __init__(self, 
                 slowcam_timestamp_path, 
                 slowcam_data_path,
                 fastcam_matched_data_name, 
                 timestamp_matching_pkl_name,
                 obs_start, obs_end,
                 match_frames = True,
        ...
        self.obs_start = obs_start
        self.obs_end = obs_end
        self.fastcam_matched_data_name = fastcam_matched_data_name
        self.timestamp_matching_pkl_name = timestamp_matching_pkl_name
        ...
        if match_frames:
            ...
            if self.psfcam_is_fastcam:
                self.match_frames(location_only = True, header = slowcam_header, footer = slowcam_footer)
            else:
    ...
    def match_frames(self, location_only = True, header = 'firstpl_', footer = '.fits'):
        ...
        plcam_timestampfiles = find_data_between(self.slowcam_timestamp_path, self.obs_start, self.obs_end, header=header, footer='.txt')
        plcam_data = find_data_between(self.slowcam_data_path, self.obs_start, self.obs_end, header=header, footer=footer)
        ...
        psfcam_frames = np.load(self.fastcam_matched_data_name)
        with open(self.timestamp_matching_pkl_name, 'rb') as f:
        ...
        if not location_only:
            ...
            all_file_indices = []
            all_frame_indices = []
            for fi, f in enumerate(plcam_data):
    ...
    def match_frames2(self, header = 'palila_', footer = '.fits',
                      crop_width = 20):
        '''
        Match the frames from the two cameras, when PSF camera is slower
        '''

        psfcam_timestampfiles = find_data_between(self.slowcam_timestamp_path, self.obs_start, self.obs_end, header=header, footer='.txt')
        psfcam_data = find_data_between(self.slowcam_data_path, self.obs_start, self.obs_end, header=header, footer=footer)
        
        
        # load timestamps
        timestamps_matching_psf = np.concatenate([np.genfromtxt(file)[:,4] for file in psfcam_timestampfiles])
        
        # load timestamp matching pkl file
        # do not load all plcam frames! this would be too large
        plcam_frames_mmap = np.load(self.fastcam_matched_data_name, mmap_mode='r')
        with open(self.timestamp_matching_pkl_name, 'rb') as f:
            plcam_timestamp = pickle.load(f)
        
        # load psfcam frame indices
        # also load psfcam frames
        all_file_indices = []
        all_frame_indices = []
        psfcam_frames  = []
        
        for fi, f in enumerate(psfcam_data):
            for n in range(fits.getheader(f)['NAXIS3']): #fits.getdata(f).shape[0]):
                all_file_indices.append(fi)
                all_frame_indices.append(n)
            data = fits.getdata(f)
            for i in range(len(data)):
                psfcam_frames.append(data[i, 
                                        data.shape[1]//2 - crop_width : data.shape[1]//2 + crop_width,
                                        data.shape[2]//2 - crop_width : data.shape[2]//2 + crop_width])
        all_file_indices = np.array(all_file_indices)
        all_frame_indices = np.array(all_frame_indices)
        psfcam_frames = np.array(psfcam_frames)
        psfcam_frames -= self.psfcam_dark[self.psfcam_dark.shape[0]//2 - crop_width : self.psfcam_dark.shape[0]//2 + crop_width,
                                       self.psfcam_dark.shape[1]//2 - crop_width : self.psfcam_dark.shape[1]//2 + crop_width]
        
        
        # validate timestamps
        idx1, idx2 = validate_timestamp_matching(timestamps_matching_psf, (np.array(plcam_timestamp['timestamps'])))
        
        # filter out the frames that don't match
        all_file_indices = all_file_indices[idx1]
        all_frame_indices = all_frame_indices[idx1]
        self.psfcam_frames = psfcam_frames[idx1]
        self.psfcam_file_indices = all_file_indices
        self.psfcam_frame_indices = all_frame_indices
        self.psfcam_files = psfcam_data
        
        plcam_frames_mmap = plcam_frames_mmap[idx2]
        self.plcam_frames_mmap = plcam_frames_mmap
        self.timestamps = np.array(plcam_timestamp['timestamps'])[idx2]
    ...
    def bin_by_centroids(self, map_n, map_width, effective_idx = None, plot = True, 
        ...
        if effective_idx is not None:
            ...
            if self.psfcam_is_fastcam:
                ...
                plcam_frames = self.plcam_frames_mmap[effective_idx]
            # else:
        ...
        if xc is None: 
            _x = centroids[:,0]
            clipped = sigma_clip(_x, sigma=3, maxiters=5)
            xc = np.nanmedian(clipped.data[~clipped.mask])
        if yc is None: 
            _y = centroids[:,1]
            clipped = sigma_clip(_y, sigma=3, maxiters=5)
            yc = np.nanmedian(clipped.data[~clipped.mask])

        xbins = np.linspace(xc - map_width/2, xc + map_width/2, map_n+1)
        ybins = np.linspace(yc - map_width/2, yc + map_width/2, map_n+1)

        self.x_mas = ((xbins[:-1] + np.diff(xbins)[0]/2) - xc) * self.pix2mas
        self.y_mas = ((ybins[:-1] + np.diff(ybins)[0]/2) - yc) * self.pix2mas
        ...
        if not to_file:
            ...
            if self.psfcam_is_fastcam:
                self.psfcam_binned_frames, self.num_frames, self.idxs = bin_by_centroids_to_file(filename, psfcam_frames, plcam_file_indices, plcam_frame_indices, self.plcam_files, self.ny, self.nx, centroids, xbins, ybins, skip_frame_reading=skip_frame_reading,
                                                                                             nbin = nbin, timestamps = timestamps)
            else:
                self.psfcam_binned_frames, self.num_frames, self.idxs = bin_by_centroids_to_file2(filename, psfcam_frames, plcam_frames, self.ny, self.nx, centroids, xbins, ybins, skip_frame_reading=skip_frame_reading,
                                                                                             nbin = nbin, timestamps = timestamps)
            # return      
