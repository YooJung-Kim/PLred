...
datadir = '/mnt/datazpool/PL/yjkim/remapped/mizarA_20250514_highres/'
...
for i in tqdm(range(25)):
    for j in range(25):
        ...
        if os.path.exists(datadir+'remapped_binned_08:31:00_08:36:59_bin_%d_%d.h5' % (i,j)):
            print('Loading %d %d' % (i,j))
            h5file = h5py.File(datadir+'remapped_binned_08:31:00_08:36:59_bin_%d_%d.h5' % (i,j), 'r')
            h = h5file['rawframes'][:]
...
np.save(datadir+f'corrected_map_mizarA2_highres_binned_{xmin}_{xmax}', corrected_map)
np.save(datadir+f'status_map_mizarA2_highres_binned_{xmin}_{xmax}', status)
